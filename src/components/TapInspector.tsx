...
export default function TapInspector() {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("touchstart", onTouch as EventListener, true);
      window.removeEventListener("pointerdown", onPointer as EventListener, true);
      window.removeEventListener("mousedown", onMouse as EventListener, true);
    };
  }, []);
  ...
  return (
    <>
      ...
      >
        {!hit ? (
          <div>
            <b>Conseil:</b> touche “Connexion”, puis envoie-moi la ligne <b>Hit/pos/z/pe</b>{" "}
            (bouton COPIER).
          </div>
        ) : (
          <>
