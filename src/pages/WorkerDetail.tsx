...
const WorkerDetail: React.FC = () => {
  ...
  // Votes sur les avis (review.id utilisé comme reply_id)
  ...
  useEffect(() => {
    setVoteError(null);
    loadVotesAndCounts().catch((e) => {
      console.error("loadVotesAndCounts error", e);
      // ne bloque pas l’écran si ça échoue
    });
  ...
  // -----------------------
  // Contact / review submit (inchangé)
  // -----------------------
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 space-y-4">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
              <h2 className="text-sm font-semibold text-slate-800 mb-1">{language === "fr" ? "Portfolio / Réalisations" : "Portfolio / Works"}</h2>
              <p className="text-xs text-slate-500 mb-2">
                {language === "fr"
                  ? "Découvrez quelques projets réalisés par ce professionnel."
                  : "Discover some projects completed by this professional."}
              </p>
              <p className="text-sm text-slate-500">{language === "fr" ? "Aucune réalisation publiée pour le moment." : "No published work yet."}</p>
            </div>
